...
- **Local bridge**: `src-tauri/src/bridge.rs` — starts a local unauthenticated SOCKS5 proxy per session that relays traffic to the authenticated upstream SOCKS5 proxy returned by a market session purchase. Commands: `bridge_start` (returns the bound local port), `bridge_stop`, `bridge_port`. Uses a global `BRIDGES` registry (session_id → Bridge with shutdown oneshot channel). Also exposes `stop_bridge_on_port()` for reclaiming a port before restart.
